...
angular.module('Giftshop').service('AuthenticationService', ['Base64', '$http', '$q', 'localStorageService', '$rootScope', function (Base64, $http, $q, localStorageService, $rootScope) {
    this.Logon = function (id, pwd) {
        return $http.post('/api/Authentication/Logon', { user_id: id, passwordx: pwd }).then(handleSuccess, handleError);
    };

    this.SetCredentials = function (response) {
        var _authdata = Base64.encode(response.user.user_id + ':' + response.user.passwordx);
        $rootScope.globals = {
            currentUser: {
                username: response.user.first_name + " " + response.user.last_name,
                siteid: response.user.site_id,
        ...
        $http.defaults.headers.common['Authorization'] = 'Shov ' + _authdata; // jshint ignore:line
    ...
    this.ClearCredentials = function () {
        ...
        $http.defaults.headers.common.Authorization = 'Shov ';
    };
