angular.module('Giftshop').controller('HomeController', function ($scope, $state, $uibModal, $uibModalStack, $mdDialog, $rootScope, $ngConfirm, inform, $interval, AuthenticationService, Idle, Keepalive, TableModalService, localStorageService, PrinterService, SusersServices) {
    $scope.userName = $rootScope.globals.currentUser.username;
    $scope.user = $rootScope.globals.User;
    site = $rootScope.globals.User.site_id;
    var NETWORKPRINT = ''
    ...
    $scope.openChangePassword = function () {
        $uibModal.open({
            size: 'sm',
            animation: true,
            templateUrl: 'Angular/Views/Home/changePassword.htm'
        });
    };
    ...
    $scope.$on('IdleStart', function () {
        ...
        $scope.warning = $uibModal.open({
            templateUrl: 'Angular/Views/Home/Idle.htm',
    ...
    $scope.prompt = function () {
        $ngConfirm({
            title: 'Replicate System',
            contentUrl: 'Angular/Views/Home/replicateSystem.htm',
            ...
            buttons: {
                Replicate: {
                    ...
                    action: function (scope) {
                        // $state.transitionTo( '.system', { SYSTEM_ID: scope.SYSTEMID } );
                        localStorageService.set('SYSTEM_ID', scope.SYSTEMID);
                        $state.go('home.system');

                        $ngConfirm({ content: 'System <strong> ' + scope.SYSTEMID + '</strong>, has been replicated successful.', title: 'Replicate Successfull', closeIcon: false, useBootstrap: false, backgroundDismiss: true, theme: 'material', bootstrapClasses: { size: 'sm' }, boxWidth: '20%', buttons: { OK: { keys: ['enter', 'esc'], text: 'OK' } } });
                    }
                },
                Close: function () {
                }
            ...
            onScopeReady: function (scope) {
                ...
                scope.textChange = function () {
                    if (scope.SYSTEMID)
                        self.buttons.Replicate.setDisabled(false);
                    else
                        self.buttons.Replicate.setDisabled(true);
                }
            }
        })
    };
    ...
    $scope.ChangePasswd = function () {
        $ngConfirm({
            ...
            contentUrl: 'Angular/Views/Home/changePassword.htm',
            backgroundDismiss: false,
            backgroundDismissAnimation: '',
            useBootstrap: false,
            boxWidth: '30%',
    ...
    $scope.ShortPasswd = function () {
        $ngConfirm({
            ...
            contentUrl: 'Angular/Views/Home/shortPassword.htm',
            backgroundDismiss: false,
            backgroundDismissAnimation: '',
            useBootstrap: false,
            boxWidth: '30%',
